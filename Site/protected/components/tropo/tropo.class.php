...
class Session {
    ...
    public function __construct($json = NULL) {
        ...
        $this->_to = isset($session->session->to) ? array(
        ...
        $this->_from = isset($session->session->from->id) ? array(
        ...
        $this->_headers = isset($session->session->headers) ? self::setHeaders($session->session->headers) : array();
